...
- 登记题目的测试用例发生新增、编辑或删除后，自动让 AI 更新 `main.<语言后缀>`。AI 正在提取用例或生成测试代码时，所有用例编辑操作都会被锁定。
- 单独运行一个用例或运行全部用例，并查看预期输出与实际输出；不同的输出行会以黄色高亮。
...
2. 点击 `...`，选择“从 VSIX 安装…” 。
3. 选择 `leetcode-cph-receiver-0.6.1.vsix`（或你构建的更新版本）。
4. 安装后重新加载 VS Code。
...
4. 点击 **+新增测试用例** 创建空白用例，然后填写输入和预期输出；也可以编辑或删除已有用例。每次变化都会让所选 AI 更新 `main.<语言后缀>`。
...
- 点击用例上的 **运行**，只运行当前用例。
- 点击 **运行全部测试用例**，运行全部用例。
- 在用例卡片中查看预期输出和实际输出；不一致的行会以黄色高亮。

AI 生成的 `main.<语言后缀>` 是可执行代码。请只在受信任的工作区运行，并在首次运行提示中先审阅代码再确认。

如果 AI 生成的 `main.<语言后缀>` 出现可修复的编译、运行或结果协议错误，并且 AI Key 仍然有效，插件会把当前解答、已有的 `main.<语言后缀>`、全部测试用例以及经过清理和长度限制的诊断信息发送给所选 AI 进行修复。替换代码会先经过校验，旧的 `main.<语言后缀>` 也会备份。插件不会静默重新运行测试；请先审阅更新后的文件，再手动运行一次。缺少本机编译器或运行时属于工具链配置问题，不会触发 AI 修复。
